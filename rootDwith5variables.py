...
P = dist.Field(name='K', bases=(xbasis,ybasis))
...
problem = d3.IVP([u, v, H, P,F], namespace=locals())

problem.add_equation(
    "dt(u)+c**2/6*dx(F)+g_accel*dx(H)=" \
    "-c**2/6*dx(-P**2/(4*H**2))" \
    "-u*dx(u)-v*dy(u)+g_accel*dx(b))",condition="(nx!=0) or (ny!=0)"
    )

problem.add_equation(
    "dt(v)+c**2/6*dy(F)+g_accel*dy(H)=" \
    "-c**2/6*dy(-P**2/(4*H**2))" \
    "-u*dx(v)-v*dy(v)+g_accel*dy(b)",condition="(nx!=0) or (ny!=0)"
    )

problem.add_equation(
    "dt(v)=0",condition="(nx==0) and (ny==0)"
    )
problem.add_equation(
    "dt(u)=0",condition="(nx==0) and (ny==0)"
    )

problem.add_equation(
    "dt(H)-P=0.1*(dx(dx(H))+dy(dy(H)))"
    )
